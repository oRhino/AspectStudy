...
//懒加载，返回的是原始的invocation里面的aspects参数数组
...
//代表一个Aspect的具体信息：包括被Hook的对象，SEL，插入的block等具体信息。
...
//一个并查集，传入一个selectorName，通过递归查找，找到所有包含这个selectorName的set，最后把这些set合并在一起作为返回值返回。
...
static id aspect_add(id self, SEL selector, AspectOptions options, id block, NSError **error) {
    ...
    aspect_performLocked(^{
        ...
        if (aspect_isSelectorAllowedAndTrack(self, selector, options, error)) {
            ...
            //创建AspectsContainer对象，以aspects_ + SEL为key，作为关联对象依附到被hook的对象上
            AspectsContainer *aspectContainer = aspect_getContainerForObject(self, selector);
            
            //创建AspectIdentifier对象，并且添加到AspectsContainer对象里存储起来
            //------1:生成block的方法签名NSMethodSignature
            //------2:对比block的方法签名和待hook的方法签名是否兼容（参数个数，按照顺序的类型）
...
static void aspect_performLocked(dispatch_block_t block) {
    //如果一个低优先级的线程获得锁并访问共享资源，这时一个高优先级的线程也尝试获得这个锁，它会处于 spin lock 的忙等(busy-wait)状态从而占用大量 CPU。此时低优先级线程无法与高优先级线程争夺 CPU 时间，从而导致任务迟迟完不成、无法释放 lock
...
//判断block的方法签名和目标的selector方法签名是否匹配
...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    ...
    if (!aspect_isMsgForwardIMP(targetMethodIMP)) {
        ...
        //动态生成的子类 selector -> objc_msgForward
...
//    新建一个子类，ViewController_Aspects_,并且按照上述的方式hook forwoardInvocation:
//    hook _Aspects_ViewController的class方法，让其返回ViewController
//    hook _Aspects_ViewController的类对象的isa，让其返回ViewController
//    调用objc_setClass来修改ViewController的类isa为ViewController_Aspects_
//    这样做，就可以通过object_getClass(self)获得类名，然后看看是否有前缀类名来判断是否被hook过了

    
    // Default case. Create dynamic subclass.
    //XXX_Aspects_ 动态生成一个子类
	...
	if (subclass == nil) {
        ...
        //hook forward方法  forwardInvocation -> __ASPECTS_ARE_BEING_CALLED__
        // __aspects_forwardInvocation -> forwardInvocation的IMP
		aspect_swizzleForwardInvocation(subclass);
        
        //hook class subclass.class -> 父类对象的class
		aspect_hookedGetClass(subclass, statedClass);
        //XXX_Aspects_类对象 class -> 父类对象的class
...
//替换类的forwardInvocation:的实现为__ASPECTS_ARE_BEING_CALLED__
//如果存在就添加__aspects_forwardInvocation: 指向原来的forwardInvocation:
...
static void __ASPECTS_ARE_BEING_CALLED__(__unsafe_unretained NSObject *self, SEL selector, NSInvocation *invocation) {
    ...
    if (objectContainer.insteadAspects.count || classContainer.insteadAspects.count) {
        ...
        //获取被hook的对象的class,返回的是子类
        Class klass = object_getClass(invocation.target);
        do {
            //调用被hook的方法
            //添加前缀selector -> 被hook的方法的IMP
...
//创建AspectsContainer对象，以aspects_ + SEL为key，作为关联对象依附到被hook 的对象上
...
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
    ...
    static dispatch_once_t pred;
    //初始化集合,不允许hook的方法@"retain", @"release", @"autorelease", @"forwardInvocation:",添加黑名单
    dispatch_once(&pred, ^{
    ...
    // Additional checks.
    //dealloc只允许在AspectPositionBefore模式
    ...
    if (class_isMetaClass(object_getClass(self))) {
        ...
        //判断当前tracker的subclass里面是否包含selectorName。因为一个方法在一个类的层级里面只能被hook一次。如果已经tracker里面已经包含了一次，那么会报错。
...
//把methodSignature方法签名里面的参数，都加入到数组里，最后把数组返回。
- (NSArray *)aspects_arguments {
